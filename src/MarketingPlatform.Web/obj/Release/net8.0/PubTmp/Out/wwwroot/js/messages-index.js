...
function initMessagesTable() {
    messagesTable = $('#messagesTable').DataTable({
        ...
        // AJAX configuration
        ajax: {
            url: '/Messages/GetMessages', // SERVER-SIDE: Call Web controller
            type: 'POST',
        ...
        columns: [
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    if (type === 'display') {
                        var contact = row.contactName || '';
                        var line2 = contact ? '<br/><small class="text-muted">' + escapeHtml(contact) + '</small>' : '';
                        return escapeHtml(data || 'N/A') + line2;
                    }
                    return data;
                }
            },
            {
                ...
                render: function(data, type, row) {
                    if (type === 'display') {
                        var statusNames = ['Pending', 'Queued', 'Sending', 'Sent', 'Delivered', 'Failed', 'Bounced', 'Rejected'];
                        var statusColors = [  'warning', 'info',   'primary','success','success',   'danger','danger', 'danger'];
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    if (type === 'display') {
                        var opens = row.openCount || 0;
                        var clicks = row.clickCount || 0;
                        if (opens === 0 && clicks === 0) return '<span class="text-muted">—</span>';
                        return '<small><i class="bi bi-eye me-1"></i>' + opens + ' <i class="bi bi-cursor ms-2 me-1"></i>' + clicks + '</small>';
                    }
                    return '';
                }
            },
        ...
        order: [[6, 'desc']], // Sort by sent date descending (column index 6)
...
function duplicateMessage(id) {
    confirmAction('Are you sure you want to duplicate this message?', function() {
        $.ajax({
            url: `/Messages/Duplicate?id=${id}`, // SERVER-SIDE: Call Web controller
            method: 'POST',
            success: function(response) {
                if (response.success || response.isSuccess) {
                    showNotification(response.message || 'Message duplicated successfully!', 'success');
