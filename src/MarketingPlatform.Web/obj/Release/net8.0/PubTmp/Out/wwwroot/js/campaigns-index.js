...
function initCampaignsTable() {
    campaignsTable = $('#campaignsTable').DataTable({
        ...
        // AJAX configuration
        ajax: {
            url: '/Campaigns/GetCampaigns', // SERVER-SIDE: Web controller handles API call
            type: 'POST',
            contentType: 'application/json',
            data: function(d) {
                // Map DataTables column index to column name for server-side sorting
                const columnNames = ['name', 'type', 'status', 'scheduledAt', 'createdAt'];
                const order = d.order && d.order.length > 0 ? d.order.map(function(o) {
                    return {
                        column: columnNames[o.column] || 'createdAt',
                        dir: o.dir || 'desc'
                    };
...
function renderActionButtons(id, row) {
    ...
    if (row.status === CampaignStatus.Draft || row.status === CampaignStatus.Scheduled || row.status === CampaignStatus.Paused) {
        buttons += `
            <button class="btn btn-outline-info" onclick="editCampaign(${id})" title="Edit">
...
/**
 * Navigate to campaign edit page (Details page with edit capability)
 */
...
function duplicateCampaign(id) {
    confirmAction('Are you sure you want to duplicate this campaign?', function() {
        $.ajax({
            url: `/Campaigns/Duplicate?id=${id}`, // SERVER-SIDE
            method: 'POST',
...
function startCampaign(id) {
    confirmAction('Are you sure you want to start this campaign?', function() {
        $.ajax({
            url: `/Campaigns/Start?id=${id}`, // SERVER-SIDE
            method: 'POST',
            headers: getAjaxHeaders(),
            success: function(response) {
                if (response.success || response.isSuccess) {
...
function pauseCampaign(id) {
    confirmAction('Are you sure you want to pause this campaign?', function() {
        $.ajax({
            url: `/Campaigns/Pause?id=${id}`, // SERVER-SIDE
            method: 'POST',
            headers: getAjaxHeaders(),
            success: function(response) {
                if (response.success || response.isSuccess) {
...
function resumeCampaign(id) {
    confirmAction('Are you sure you want to resume this campaign?', function() {
        $.ajax({
            url: `/Campaigns/Resume?id=${id}`, // SERVER-SIDE
            method: 'POST',
            headers: getAjaxHeaders(),
            success: function(response) {
                if (response.success || response.isSuccess) {
...
function cancelCampaign(id) {
    confirmAction('Are you sure you want to cancel this campaign? This action cannot be undone.', function() {
        $.ajax({
            url: `/Campaigns/Cancel?id=${id}`, // SERVER-SIDE
            method: 'POST',
            headers: getAjaxHeaders(),
            success: function(response) {
                if (response.success || response.isSuccess) {
...
function deleteCampaign(id) {
    confirmAction('Are you sure you want to delete this campaign? This action cannot be undone.', function() {
        $.ajax({
            url: `/Campaigns/Delete?id=${id}`, // SERVER-SIDE
            method: 'POST',
            headers: getAjaxHeaders(),
            success: function(response) {
                if (response.success || response.isSuccess) {
